            ...
            for k in range(n_batch):
                ...
                for t in range(1, self.max_T + 1):
                    ...
                    if t == 1:
                        ...
                        kl = torch.stack([F.cross_entropy(pred_logits[u][:l] + eps, xs[u][:l].long(), reduction="mean") for u, l in enumerate(lengths)])
                    elif t == self.max_T:
                        kl += torch.stack([F.kl_div(pred_logits[u][:l] + eps, true_probs[u][:l], reduction="batchmean") for u, l in enumerate(lengths)])
                        ...
                        kl += torch.stack([F.kl_div(x_distr_padded[u][:l] + eps, true_prior[u][:l], reduction="batchmean") for u, l in enumerate(lengths)])
                    else:
                        kl += torch.stack([F.kl_div(pred_logits[u][:l] + eps, true_probs[u][:l], reduction="batchmean") for u, l in enumerate(lengths)])
                # kl /= self.max_T
                kl = kl / math.log(2)
